...
class View
{
    ...
    public function __construct()
	{
        ...
        $this->tplDir = DIR_THEME_APP . $this->config['THEME'] . DS;
		$this->compiledDir = DIR_COMPILED_APP;
    }
    ...
    public function render($tplName) 
	{
		...
		// 没有模板名
		if(!$tplName){
			$tplName = $this->config['URI']['controller']. '.' . $this->config['URI']['action'];
		};
	...
	protected function parse($tpl)
	{
		...
		$tpl = preg_replace("/\{url\s+(\S+)\}/","<?php echo Helper::getUrl('\\1');?>",$tpl);
		...
		$tpl = preg_replace("/\{(\\$[a-zA-Z0-9_\[\]\'\"\$\x7f-\xff]+)\}/es", "Helper::addquote('<?php echo \\1;?>')",$tpl);
